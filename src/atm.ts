...
const readDataFromFile = (filename: string): Account[] | null => {
...
export async function userLogin() {
  ...
  if (accountData) {
    ...
    for (let i = 0; i < accountData.length; i++) {
      if (
        accountData[i].username === existingAccountCredentials.username &&
        ...
        const showCredentials = () => {
          ...
          console.log(chalk.blue.bold("=============================="));
          console.log(
            chalk.green.bold(`Name          : ${accountData[i].name}`)
          );
          console.log(
            chalk.green.bold(`Card Type     : ${accountData[i].type}`)
          );
          console.log(
            chalk.green.bold(`Expiry Date   : ${accountData[i].expiryDate}`)
          );
          console.log(
            chalk.green.bold(`CVV           : ${accountData[i].cvv}`)
          );
          console.log(
            chalk.green.bold(`Balance       : ${accountData[i].balance}`)
          );
          console.log(chalk.blue.bold("=============================="));
        };
        ...
        while (!exit) {
          ...
          if (optPick.action === "Deposit") {
            ...
            if (
              ...
              console.log(
                chalk.red("Invalid recipient or insufficient balance.")
              );
              await delay(4000);
            }
